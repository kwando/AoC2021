defmodule Day8 do
  ...
  def unique_outputs(examples) do
    for {_, outputs} <- examples, reduce: 0 do
      sum ->
        sum + Enum.count(outputs, fn output -> Enum.member?(@unique, length(output)) end)
    end
  end

  def part2(input) do
    for row <- input, reduce: 0 do
      sum -> sum + solve(row)
    end
  end

  defp solve({inputs, outputs}) do
    %{
      2 => [one],
      3 => [seven],
      4 => [four],
  ...
  def parse_line(line) do
    ...
    {parse_patterns(input), parse_patterns(output)}
  end
